...
float volumeMult = 1;	// Volume Multiplier to boost or shrink PWM strength (1 = Normal Value)
...
// 1286 byte compiled
	...
	uint16_t pwmWithVol = (uint16_t) (nextPwm * volumeMult);
	if (pwmWithVol > 255)
	{
		pwmWithVol = 255;
	}
	OCR2B = (uint8_t) pwmWithVol;
